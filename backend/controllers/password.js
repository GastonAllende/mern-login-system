...
exports.forgotPassword = async (req, res) => {
    ...
    users.findOne({ email }, (err, user) => {
        ...
        const token = jsonwebtoken.sign({ _id: user._id, email: user.email }, "JWT_RESET_PASSWORD", {
...
exports.resetPassword = async (req, res) => {
    ...
    if (resetPasswordToken) {
        jsonwebtoken.verify(resetPasswordToken, "JWT_RESET_PASSWORD", function (err, decoded) {
            if (err) {
                return res.json({
                    result: "error",
        ...
        await users.findOneAndUpdate({ resetPasswordToken: resetPasswordToken }, updatedFields).then((responses) => {
            return res.json({
                result: "success",
